    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <Zoom  v-if="skuInfo.skuImageList" :skuImgList="skuInfo.skuImageList"/>
          <!-- 小图列表 -->
          <!-- 借助 v-if 当确定数据存在了才渲染元素，避免读取其中某项属性报错 -->
          <ImageList v-if="skuInfo.skuImageList" :skuImgList="skuInfo.skuImageList"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                @click="checkedVal(value,spuSaleAttr.spuSaleAttrValueList)">{{value.saleAttrValueName}}</dd>
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="skuNum" @change="inputNum">
...
export default {
  ...
  methods: {
    // 组件内可以修改从 store获取过来的数据，并重新渲染
    checkedVal (val, arr) { // val：v-for生成的被点击那项，arr：v-for遍历的数组
    // 每次点击先进行 排他 处理
      arr.forEach(item => {
        item.isChecked = 0
      })
